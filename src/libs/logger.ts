...
import pino, { type LoggerOptions, type Level, type TransportTargetOptions } from 'pino';
 ...
 */
export function getTraceContext(req?: { headers: Record<string, string | string[]> }): Record<string, string> | undefined {
  let traceHeader: string | undefined;
  
  // First check if we have a request object with headers
  traceHeader = process.env['TRACE_HEADER'] || process.env['X_CLOUD_TRACE_CONTEXT'] || req?.headers['x-cloud-trace-context'] as string;
  
  if (!traceHeader) {
    logger.info('No request object provided, checking environment variables for trace context');
    return undefined;
  }
...
const httpLogger = pinoHttp({
  ...
  autoLogging: {
    ignore: (req) => {
      ...
      return ['/health', '/readiness', '/liveness'].some((path) => req.url?.includes(path));
    },
  },

  customProps: (req, res) => ({
    ... (getTraceContext(req as any) || {}),
    service: appConfig.gcp?.project ? `${appConfig.gcp.project}/${appConfig.gcp.service}` : 'local',
  }),

  redact: appConfig.server.isProduction ? ['req.headers.authorization'] : [], // Redact sensitive headers
  
  // Custom request ID generation/extraction
  genReqId: (req) => {
    // Use existing request ID if available, or generate a new one
    return req.id || req.headers['x-request-id'] || randomBytes(16).toString('hex');
  },
