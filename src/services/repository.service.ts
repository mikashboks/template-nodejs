...
import { AbstractService, BaseQuery, IService, PaginatedResult, ServiceOptions } from "./abstract.service.ts";
...
export abstract class RepositoryService<
  ...
  TCreateInput = any,
  TUpdateInput = any,
> extends AbstractService implements IRepositoryService<TModel, TQuery, TCreateInput, TUpdateInput, string> {
  ...
  public async withTransaction<T>(
    ...
    if (typeof this.repository.withTransaction !== 'function') {
      contextLogger.warn('Repository does not support transactions. Executing without transaction context.');
  ...
  public async find(
    ...
    return this.trackExecution('find', async () => {
      ...
      if (query.sortBy) {
        paginationOptions.orderBy = { [query.sortBy]: query.sortOrder || 'asc' };
      }
   ...
   */
  public async findById(id: string, options?: ServiceOptions): Promise<TModel | null> {
   ...
   */
  public async update(id: string, data: TUpdateInput, options?: ServiceOptions): Promise<TModel | null> {
    ...
    }, options, { id, dataFields: Object.keys(data as any) });
  }
   ...
   */
  public async hardDelete(id: string, options?: ServiceOptions): Promise<boolean> {
    // Check if repository supports hardDelete
    if (typeof this.repository.hardDelete !== 'function') {
      throw new Error(`Hard delete not supported by repository for service ${this.serviceName}`);
    }
   ...
   * @throws Error if not supported by repository
   */
  public async restore(id: string, options?: ServiceOptions): Promise<TModel | null> {
    // Check if repository supports restore
    if (typeof this.repository.restore !== 'function') {
      throw new Error(`Restore not supported by repository for service ${this.serviceName}`);
    }
