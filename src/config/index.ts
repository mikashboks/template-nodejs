...
const isCloudRun = async (): Promise<boolean> => {
    try {
        // First try the metadata API
        return await gcpMetadata.isAvailable();
    } catch (error) {
        // Fallback to checking K_SERVICE environment variable
...
const cacheSchema = z.object({
    ...
    CACHE_MEMORY_TTL_MS: z.coerce.number().int().positive().default(60 * 1000), // 60 seconds
    CACHE_MEMORY_LRU_SIZE: z.coerce.number().int().positive().default(5000),
    CACHE_DEFAULT_TTL_MS: z.coerce.number().int().positive().default(DEFAULT_CACHE_TTL), // 10 seconds (used by Redis store by default)
    CACHE_REFRESH_THRESHOLD_MS: z.coerce.number().int().nonnegative().default(3000), // 3 seconds
...
const envSchema = z.object({
    ...
    if (data.CACHE_ENABLED && data.CACHE_REDIS_ENABLED && !data.CACHE_REDIS_URL) {
      return false;
    }
    return true;
  }, {
    message: "REDIS_URL is required when CACHE_ENABLED and CACHE_REDIS_ENABLED are true.",
...
export async function getConfig(): Promise<AppConfig> {
    try {
        ...
        if (!envResult.success) {
            console.error('❌ Environment validation errors:', envResult.error.format());
        ...
        const cloudRunDetected = await isCloudRun();
        ...
        console.log(`Cache Config: Enabled=${config.cache.enabled}, Redis=${config.cache.redisEnabled}, Namespace='${config.cache.namespace}'`);
        if (config.cache.enabled && config.cache.redisEnabled && !config.cache.redisUrl) {
            console.warn("⚠️ Cache Warning: Redis cache is enabled but REDIS_URL is missing in the environment.");
        }
...
// Initialize config async, ensure it completes before app fully starts if possible
const configPromise = getConfig()
    .then(config => {
        cachedConfig = Object.freeze(config); // Freeze for immutability
        // Use logger if available, otherwise console
        (typeof logger !== 'undefined' ? logger : console).info('✅ Configuration initialized successfully');
        return cachedConfig;
    })
    .catch(error => {
        console.error('❌ FATAL: Failed to initialize configuration:', error);
        // Optional: Exit process if config is critical for startup
        // process.exit(1);
        // We don't set cachedConfig, so calls to config() will throw.
        // Rethrow to ensure calling code knows initialization failed
        throw error;
    });

/**
 * Synchronous function to get the config.
 * Throws if config hasn't been initialized yet.
 * Ensures config is ready before returning.
 */
export const config = (): AppConfig => {
    if (!cachedConfig) {
        // This should ideally not happen if configPromise is awaited at startup
        throw new Error('Configuration not initialized. Ensure getConfig() promise resolves before accessing config.');
    }
    return cachedConfig;
};

/**
 * Asynchronous function to get the config, awaiting initialization.
 * Useful at the application's entry point.
 */
export const awaitConfig = (): Promise<AppConfig> => {
